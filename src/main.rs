...
fn main() {
    match find_r_bin() {
        None => println!("No R executable found"),
...
mod tests {
    ...
    use crate::{dir_from_path_envvar, dir_from_r_home_envvar};
    use std::{env, fs, path::PathBuf};

    #[test]
    fn dir_from_r_home_envvar_produces_none_correctly() {
        env::set_var("R_HOME", "");
        assert_eq!(dir_from_r_home_envvar(), None);
    }
    ...
    fn dir_from_r_home_envvar_produces_path_correctly() {
        ...
        assert_eq!(dir_from_r_home_envvar(), Some(tempfile));
    }

    #[test]
    fn dir_from_path_envvar_produces_none_correctly() {
        // Haven't yet figured out how to effectively mock the PATH variable
        assert_eq!(dir_from_path_envvar(), Some(PathBuf::from("/usr/local/bin/R")));

        env::set_var("PATH", "");
        assert_eq!(dir_from_path_envvar(), None)
    }
